...
impl AgentRpcImpl {
    fn try_set_json<T: Serialize>(val: T) -> Result<sea_orm::ActiveValue<serde_json::Value>, String> {
    ...
    fn get_db() -> Result<&'static DatabaseConnection, (i64, String)> {
        DB.get().ok_or_else(|| (102, "DB not initialized".to_string()))
    }
}
    ...
    async fn report_static(&self, _token: String, data: serde_json::Value) -> serde_json::Value {
        let process_logic = async {
            ...
            let in_data = static_monitoring::ActiveModel {
                ...
                timestamp: Set(parsed.time as i64),
